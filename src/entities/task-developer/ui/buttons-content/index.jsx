...
export default function ButtonsContent(props) {
  ...
  const onSendTask = async () => {
    try {
      await props.sendTask.handleSendTask(props.taskDeveloper.id, props.fileValue);
      props.refetch();
    } catch (error) {
    ...
    <>
      <div className="style-buttons-content">
        <TopIconButton
          ...
          leftIcon={props.sendTask.isLoading ? <Loading className="buying-task-spinner" /> : <CheckIcon />}
        >
